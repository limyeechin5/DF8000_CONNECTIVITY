...
--------------------------------------------------------------------------------
| 14 | 04 Jul 25 | V1.13   |  Updated exportreport
--------------------------------------------------------------------------------
...
class MainDialog(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        version = "Version 1.13 - MYSQL (04_JUL_2025)"
        ...
        self.setWindowTitle("DSCADA DF8000 CA [" + version + "]")
    ...
    def export2xlsx(self):
        try:
            ...
            # Updated headers and their dictionary keys
            selected_columns = [
                ("Subs", "Subs"),
                ("DC", "DC"),
                ("Offline (hr)", "Total_Offline(s)"),
                ("Online (hr)", "Total_Online(s)"),
                ("Availability", "Availability")
            ]

            # Write headers
            for col_idx, (header_label, _) in enumerate(selected_columns):
                summary_ws.write(0, col_idx, header_label)

            # Write data
            for row_idx, sub_code in enumerate(self.DICTIONARY, start=1):
                data = self.DICTIONARY[sub_code]
                for col_idx, (_, key) in enumerate(selected_columns):
                    value = data.get(key, "NA")

                    if key in ["Total_Offline(s)", "Total_Online(s)"]:
                        try:
                            value = float(value) / 3600  # Convert seconds to hours
                            summary_ws.write(row_idx, col_idx, round(value, 2))
                        except:
                            summary_ws.write(row_idx, col_idx, "NA")
